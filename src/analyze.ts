...
/**
 * Given the identifier that names a styles hook (e.g. `useStyles`), finds
 * every place the hook is *called* anywhere in the project (cross-file,
 * cross-package), figures out what local name the returned `classes` object
 * got bound to at each call site, and unions up every class key that's
 * actually accessed off of it — wherever that access happens to live.
 */
export function analyzeHookUsage(hookNameNode: Identifier): UsageAnalysis {
